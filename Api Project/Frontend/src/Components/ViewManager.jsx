...
export default function ViewManager() {
  ...
  const [loading, setLoading] = useState(true) // Loading state add karya
  ...
  const fetchData = async () => {
    ...
    await axios.get("http://localhost:1008/manager-show", {
      headers : {
        Authorization : token
      }
    }).then((res) => {
  ...
  const handleDelete = async (id) => {
    ...
    await axios.patch("http://localhost:1008/manager-delete",{id : id},
      {
        headers : {
          Authorization : token
        }
    ...
    <div className="w-full px-4 py-6">
      ...
      `}>
        ...
        <div className="mb-5 transition-opacity duration-500">
          <h1 className="text-2xl font-bold text-gray-900">Manager List</h1>
          <p className="text-sm text-gray-500 mt-1 transition-opacity duration-500 delay-100">
            All managers with their basic account details.
          </p>
        </div>
          ...
          <div className="overflow-x-auto transition-opacity duration-500 delay-150">
            {record.length === 0 ? (
              ...
              <div className="text-center py-12 transition-opacity duration-500">
                ...
                <p className="text-sm text-gray-500">No manager accounts have been created yet.</p>
              </div>
            ) : (
              // Table with data
              <table className="w-full border-collapse rounded-lg overflow-hidden">
                ...
                <tbody>
                  {record.map((e, index) => (
                    ...
                    >
                      <td className="py-3 px-4 text-gray-800 transition-colors duration-300">{e.username}</td>
                      <td className="py-3 px-4 text-gray-700 transition-colors duration-300">{e.phone}</td>
                      <td className="py-3 px-4 text-gray-700 transition-colors duration-300">{e.email}</td>
